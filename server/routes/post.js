...
router.post("/createpost", middleware, async (req, res) => {
  try {
    const { url, pic, place, area, no_of_bedrooms, no_of_bathrooms, quote } =
      req.body;
    if (!quote || !place || !area || !no_of_bedrooms || !no_of_bathrooms) {
...
router.delete("/deletepost/:postId", middleware, async (req, res) => {
  try {
    const updated = await Post.findByIdAndDelete(
...
router.get("/viewprofile/:id", middleware, async (req, res) => {
  try {
    const userdetails = await User.findById(req.params.id);
...
router.get("/viewposts/:id", middleware, async (req, res) => {
  try {
